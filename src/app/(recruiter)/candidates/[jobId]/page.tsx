...
interface Candidate {
  ...
  // NEW: GitHub username (not a full URL) — build the link as
  // https://github.com/{github_username}. May be null if unset.
          ...
          <div className="space-y-3">
            {filtered.map((c) => {
              ...
              return (
                ...
                // we can nest the resume/GitHub icon <button>/<a> inside
                // without invalid button-in-button HTML. Keyboard
                // accessibility (Enter/Space) is preserved via onKeyDown.
                <div
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 flex-wrap">
                        ...
                        {/* NEW: GitHub badge next to the resume badge — only
                            rendered when the candidate has a GitHub username. */}
                        {c.github_username && (
                          <a
                            ...
                            title="View GitHub"
                            className="flex items-center gap-1 px-2 py-0.5 rounded-full bg-gray-100 text-gray-600 hover:bg-gray-200 transition-colors flex-shrink-0 text-[11px] font-bold"
                          >
        ...
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center sm:p-4">
          ...
          <div className="relative w-full sm:max-w-lg h-full sm:h-auto sm:max-h-[90vh] bg-white rounded-t-[32px] sm:rounded-[32px] shadow-2xl flex flex-col overflow-hidden">
            ...
            <div className="flex-1 overflow-y-auto px-6 py-5 space-y-6">
              ...
              {/* Resume + GitHub — EN-013: resume opens the same-page modal
                  instead of navigating to a new browser tab. GitHub opens in
                  a new tab since it's an external site. Shown together in one
                  row when either exists. */}
              {(selected.resume_url || selected.github_username) && (
                <Section title="Links">
                  <div className="flex items-center gap-4">
                    {selected.resume_url && (
                      <button
                    ...
                    {selected.github_username && (
                      <a
                        ...
                        target="_blank"
                        rel="noopener noreferrer"
                        className="inline-flex items-center gap-2 text-sm font-bold text-gray-700 hover:underline"
                      >
