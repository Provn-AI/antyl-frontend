...
function ScoreBar({ score, muted }: { score: number; muted?: boolean }) {
  ...
  return (
    <div className="flex items-center gap-2 mt-3">
      ...
      <span className={`text-xs font-bold tabular-nums ${muted ? "text-gray-300" : ""}`} style={muted ? undefined : { color }}>
        {score}% match
      </span>
    </div>
  );
...
export default function ApplicationsPage() {
  ...
  const filteredApplications = applications
    ...
    // On the "All" tab, rejected applications sink to the bottom instead of
    // being interleaved with active/matched ones by date — they're closed
    // out, so they shouldn't compete for attention at the top of the list.
    .sort((a, b) => {
      ...
      <div className="px-4 py-12 md:ml-56">
        <div className="w-full max-w-2xl mx-auto">
          ...
          {/* Filter tabs — equal-width on mobile so all 4 fit in one row
              without overflowing or wrapping; auto-width from sm: up. */}
          ...
          {filteredApplications.length === 0 ? (
            ...
            <div className="space-y-3">
              {filteredApplications.map((app, index) => {
                ...
                // Divider before the first rejected card when they're
                // trailing behind active/matched ones on the "All" tab —
                // makes the grouping read as intentional, not random.
                  ...
                  <div key={app.application_id}>
                    {showRejectedDivider && (
                      <div className="flex items-center gap-3 pt-2 pb-3">
                        <span className="text-[11px] font-bold text-gray-300 uppercase tracking-wide whitespace-nowrap">
                          Not moving forward
                        </span>
                    ...
                    <div
                      className={`bg-white rounded-[24px] border p-5 sm:p-6 transition-opacity ${
                        isRejected
                          ? "border-gray-100 shadow-none opacity-60"
                          : "border-gray-100 shadow-sm"
                      }`}
                    >
                      <div className="flex items-start justify-between gap-3">
                        <div className="flex-1 min-w-0">
                          <p className={`font-bold truncate ${isRejected ? "text-gray-500" : "text-gray-900"}`}>
                            {app.job_title}
                          </p>
                      ...
                      {/* Score bar */}
                      <ScoreBar score={app.similarity_score} muted={isRejected} />

                      <div className="flex items-center justify-between mt-4">
                        {/* Via pill */}
                        <span className={`flex items-center gap-1 text-xs font-semibold rounded-full px-2.5 py-1 border ${
                          isRejected
                          ...
                          {app.applied_via === "manual" ? (
                            ...
                            <Zap className={`w-3 h-3 ${isRejected ? "text-gray-300" : "text-[#F2754A]"}`} />
                          )}
